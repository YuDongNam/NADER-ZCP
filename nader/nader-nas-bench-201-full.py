...
# =============================================================================
# Full Data Retrain (NAS-Bench-201 Protocol - Final Step)
# =============================================================================

def run_full_data_retrain(nader, best_model_name, log_dir, runs_log_path, dataset='cifar10', epochs=200, seed=888):
    """
    Best model을 full data로 retrain하여 test accuracy 획득
    
    NAS-Bench-201 Protocol:
    - Search: 25k train / 25k val split
    - Retrain: 50k train (full data) / 10k test
    ...
    Returns:
        final_test_acc: Test accuracy from full data training
    """
    import glob
    import re
    
    print_and_save(f"\n{'='*70}", runs_log_path)
    print_and_save("FINAL STEP: Full Data Retrain (NAS-Bench-201 Protocol)", runs_log_path)
    print_and_save(f"{'='*70}", runs_log_path)
    print_and_save(f"Best Model: {best_model_name}", runs_log_path)
    print_and_save(f"Dataset: {dataset}", runs_log_path)
    print_and_save(f"Epochs: {epochs}", runs_log_path)
    
    # Paths
    code_dir = nader.code_dir
    retrain_output_dir = os.path.join(os.path.dirname(nader.train_log_dir), f"{dataset}_retrain")
    os.makedirs(retrain_output_dir, exist_ok=True)
    
    # 1. Find and copy model code (with required blocks)
    print_and_save(f"\n[Step 1] Finding and copying model code...", runs_log_path)
    
    model_py = os.path.join(code_dir, f"{best_model_name}.py")
    if not os.path.exists(model_py):
        # Search for model code
        search_base = os.path.dirname(os.path.dirname(nader.train_log_dir))  # logs/nas-bench-201/{dataset}/
        pattern = os.path.join(search_base, '**', 'codes', 'models', 'code', f"{best_model_name}.py")
        matches = glob.glob(pattern, recursive=True)
        if matches:
            shutil.copy2(matches[0], model_py)
            print_and_save(f"  Copied model: {matches[0]} -> {model_py}", runs_log_path)
        else:
            print_and_save(f"  ERROR: Could not find model code for {best_model_name}", runs_log_path)
            return -1
    else:
        print_and_save(f"  Model code already exists: {model_py}", runs_log_path)
    
    # Parse required blocks from model file
    with open(model_py, 'r') as f:
        content = f.read()
    
    block_pattern = r"Registers\.block\['([^']+)'\]"
    required_blocks = re.findall(block_pattern, content)
    
    if required_blocks:
        print_and_save(f"  Required blocks: {required_blocks}", runs_log_path)
        search_base = os.path.dirname(os.path.dirname(nader.train_log_dir))
        
        for block_name in required_blocks:
            block_py = os.path.join(code_dir, f"{block_name}.py")
            if os.path.exists(block_py):
                continue
            
            # Determine block type
            if '_base' in block_name:
                search_subdir = 'blocks/base/code'
            elif '_stem' in block_name or block_name.endswith('stem'):
                search_subdir = 'blocks/stem/code'
            elif '_downsample' in block_name or block_name.endswith('downsample'):
                search_subdir = 'blocks/downsample/code'
            else:
                search_subdir = 'blocks/*/code'
            
            pattern = os.path.join(search_base, '**', search_subdir, f"{block_name}.py")
            matches = glob.glob(pattern, recursive=True)
            if matches:
                shutil.copy2(matches[0], block_py)
                print_and_save(f"  Copied block: {block_name}", runs_log_path)
            else:
                print_and_save(f"  WARNING: Could not find block {block_name}", runs_log_path)
    
    # 2. Generate training script
    print_and_save(f"\n[Step 2] Generating training script...", runs_log_path)
    
    train_script_map = {
        'cifar10': 'train_cifar10.py',
        'cifar100': 'train_cifar100.py',
        'imagenet16-120': 'train_imagenet16_120.py',
    }
    train_script = train_script_map.get(dataset, 'train_cifar10.py')
    
    script_content = f"""#!/bin/bash
# Full Data Retrain Script for {best_model_name}
# Generated by nader-nas-bench-201-full.py

cd $(dirname $(dirname $(dirname $(readlink -f $0))))
export PYTHONPATH=$(pwd):$PYTHONPATH

echo "================================================"
echo "[RETRAIN] Model: {best_model_name}"
echo "[RETRAIN] Dataset: {dataset}"
echo "[RETRAIN] Using Full Data (50k train / 10k test)"
echo "================================================"

python nader/{train_script} \\
...
echo "[RETRAIN] Training Complete"
"""
    
    script_path = os.path.join(retrain_output_dir, f"retrain_{best_model_name}.sh")
    with open(script_path, 'w') as f:
        f.write(script_content)
    print_and_save(f"  Script generated: {script_path}", runs_log_path)
    
    # 3. Execute training
    print_and_save(f"\n[Step 3] Executing full data training...", runs_log_path)
    
    stdout_log = os.path.join(retrain_output_dir, "retrain_stdout.log")
    stderr_log = os.path.join(retrain_output_dir, "retrain_stderr.log")
    ...
    with open(stdout_log, 'w') as stdout_file, open(stderr_log, 'w') as stderr_file:
        ...
        # Monitor training
        model_log_dir = os.path.join(retrain_output_dir, best_model_name, '1')
        while process.poll() is None:
            time.sleep(60)  # Check every minute
            
            # Check progress
            val_acc_file = os.path.join(model_log_dir, 'fulldata_acc.txt')
            if os.path.exists(val_acc_file):
                try:
                    with open(val_acc_file, 'r') as f:
                        lines = f.readlines()
                        if len(lines) > 1:
                            ...
                            print_and_save(f"  Progress: {last_line}", runs_log_path)
                except:
                    pass
    ...
    if ret_code != 0:
        print_and_save(f"  ERROR: Training failed with code {ret_code}", runs_log_path)
        print_and_save(f"  Check stderr: {stderr_log}", runs_log_path)
        return -1
    
    # 4. Read final test accuracy
    print_and_save(f"\n[Step 4] Reading final test accuracy...", runs_log_path)
    
    test_acc_file = os.path.join(model_log_dir, 'test_acc.txt')
    fulldata_acc_file = os.path.join(model_log_dir, 'fulldata_acc.txt')
    
    final_test_acc = -1
    
    # Try test_acc.txt first
    if os.path.exists(test_acc_file):
        try:
            with open(test_acc_file, 'r') as f:
                lines = f.readlines()
                if len(lines) >= 2:
                    last_line = lines[-1].strip()
                    parts = last_line.split(',')
                    final_test_acc = float(parts[1])
                    print_and_save(f"  Test Accuracy (from test_acc.txt): {final_test_acc:.2f}%", runs_log_path)
        except Exception as e:
            print_and_save(f"  Error reading test_acc.txt: {e}", runs_log_path)
    
    # Try fulldata_acc.txt if test_acc not found
    if final_test_acc == -1 and os.path.exists(fulldata_acc_file):
        try:
            with open(fulldata_acc_file, 'r') as f:
                lines = f.readlines()
                if len(lines) >= 2:
                    # Find best accuracy
                    best_acc = -1
                    ...
                    print_and_save(f"  Best Accuracy (from fulldata_acc.txt): {final_test_acc:.2f}%", runs_log_path)
        except Exception as e:
            print_and_save(f"  Error reading fulldata_acc.txt: {e}", runs_log_path)
    
    print_and_save(f"\n{'='*70}", runs_log_path)
    print_and_save(f"FULL DATA RETRAIN COMPLETE", runs_log_path)
    print_and_save(f"Final Test Accuracy: {final_test_acc:.2f}%", runs_log_path)
    print_and_save(f"Results saved to: {retrain_output_dir}", runs_log_path)
    print_and_save(f"{'='*70}\n", runs_log_path)
...
if __name__=='__main__':
    ...
    # === FINAL STEP: Full Data Retrain ===
    # NAS-Bench-201 Protocol: Best model을 full data로 retrain하여 최종 test accuracy 획득
    if overall_best_model:
        final_test_acc = run_full_data_retrain(
            nader=nader,
            best_model_name=overall_best_model,
            log_dir=log_dir,
        ...
        # Final report
        print_and_save(f"\n{'#'*70}", runs_log_path)
        print_and_save(f"# FINAL RESULTS (NAS-Bench-201 Protocol)", runs_log_path)
        print_and_save(f"{'#'*70}", runs_log_path)
        print_and_save(f"Best Model: {overall_best_model}", runs_log_path)
        print_and_save(f"Search Val Acc: {overall_best_val_acc:.2f}%", runs_log_path)
        print_and_save(f"Full Data Test Acc: {final_test_acc:.2f}%", runs_log_path)
        print_and_save(f"{'#'*70}\n", runs_log_path)
    else:
        print_and_save(f"\nWARNING: No best model found, skipping full data retrain.", runs_log_path)
